...
export default class UserInterface extends GUI {
  constructor(thumbRaiser) {
    ...
    // TODO modify to adjust Controls placement
    this.domElement.style.position = 'absolute';
    // this.domElement.style.right = '0.5vw';
    // this.domElement.style.top = '1.0vh';
    this.domElement.style.top = thumbRaiser.canvasContainer.offsetTop + 2 + 'px';
    this.domElement.style.left = thumbRaiser.canvasContainer.getBoundingClientRect().right - this.domElement.clientWidth - 1 + 'px';
    this.domElement.style.fontSize = fontSize;

    thumbRaiser.window.addEventListener('resize', (event) => {
      this.domElement.style.top = thumbRaiser.canvasContainer.offsetTop + 2 + 'px';
      this.domElement.style.left = thumbRaiser.canvasContainer.getBoundingClientRect().right - this.domElement.clientWidth - 1 + 'px';
    });
    ...
    audioFolder
      .add(thumbRaiser.audio, 'volume', thumbRaiser.audio.volumeMin, thumbRaiser.audio.volumeMax, thumbRaiser.audio.volumeStep)
    ...
    directionalLightFolder
      .add(thumbRaiser.directionalLight.orientation, 'h', thumbRaiser.directionalLight.orientationMin.h, thumbRaiser.directionalLight.orientationMax.h, thumbRaiser.directionalLight.orientationStep.h)
      .onChange((h) => positionCallback(thumbRaiser.directionalLight, thumbRaiser.directionalLight.distance, new Orientation(h, thumbRaiser.directionalLight.orientation.v)));
    directionalLightFolder
      .add(thumbRaiser.directionalLight.orientation, 'v', thumbRaiser.directionalLight.orientationMin.v, thumbRaiser.directionalLight.orientationMax.v, thumbRaiser.directionalLight.orientationStep.v)
      .onChange((v) => positionCallback(thumbRaiser.directionalLight, thumbRaiser.directionalLight.distance, new Orientation(thumbRaiser.directionalLight.orientation.h, v)));
    ...
    spotLightFolder
      .add(spotLightParameters, 'angle', thumbRaiser.spotLight.angleMin, thumbRaiser.spotLight.angleMax, thumbRaiser.spotLight.angleStep)
      .onChange((angle) => (thumbRaiser.spotLight.angle = THREE.MathUtils.degToRad(angle)));
    ...
    flashLightFolder
      .add(flashLightParameters, 'angle', thumbRaiser.flashLight.angleMin, thumbRaiser.flashLight.angleMax, thumbRaiser.flashLight.angleStep)
      .onChange((angle) => (thumbRaiser.flashLight.angle = THREE.MathUtils.degToRad(angle)));
    ...
    flashLightFolder
      .add(thumbRaiser.flashLight.orientation, 'h', thumbRaiser.flashLight.orientationMin.h, thumbRaiser.flashLight.orientationMax.h, thumbRaiser.flashLight.orientationStep.h)
      .onChange((h) => positionCallback(thumbRaiser.flashLight, thumbRaiser.flashLight.distance, new Orientation(h, thumbRaiser.flashLight.orientation.v)));
    flashLightFolder
      .add(thumbRaiser.flashLight.orientation, 'v', thumbRaiser.flashLight.orientationMin.v, thumbRaiser.flashLight.orientationMax.v, thumbRaiser.flashLight.orientationStep.v)
      .onChange((v) => positionCallback(thumbRaiser.flashLight, thumbRaiser.flashLight.distance, new Orientation(thumbRaiser.flashLight.orientation.h, v)));
    ...
    fogFolder
      .add(this.fogParameters, 'density', thumbRaiser.fog.densityMin, thumbRaiser.fog.densityMax, thumbRaiser.fog.densityStep)
  ...
  setVisibility(visible) {
    if ('show' in this && 'hide' in this) {
      ...
      if (visible) {
        ...
        // this.domElement.style.backgroundColor = 'rgba(200, 0, 0, 0.5)';
      } else {
