...
export default class Camera {
    constructor(parameters, canvasContainer) {
    ...
    snapViewport(frame) { // drag: "none"; resize: string identifying the pointed frame
        ...
        if (frame == "none" || frame.includes("west")) {
            west = { size: this.window.innerWidth, currentPosition: this.viewport.x };
            this.snapPosition(west);
        }
        if (frame == "none" || frame.includes("east")) {
            east = { size: this.window.innerWidth, currentPosition: (this.viewport.x + this.viewport.width) };
        ...
        if (frame == "none" || frame.includes("south")) {
            south = { size: this.window.innerHeight, currentPosition: this.viewport.y };
            this.snapPosition(south);
        }
        if (frame == "none" || frame.includes("north")) {
            north = { size: this.window.innerHeight, currentPosition: (this.viewport.y + this.viewport.height) };
    ...
    dragViewport(mouse) {
        ...
        this.viewport.x = THREE.MathUtils.clamp(this.previousViewport.x + increment.x, 0, this.window.innerWidth - this.viewport.width);
        this.viewport.y = THREE.MathUtils.clamp(this.previousViewport.y + increment.y, 0, this.window.innerHeight - this.viewport.height);
        this.snapViewport("none");
        this.viewport = this.roundViewport(this.viewport);
        this.currentViewport = this.pixelsToPercentage(this.viewport, this.window.innerWidth, this.window.innerHeight);
    }
    ...
    resizeViewport(frame, mouse) {
        ...
        switch (frame) {
            ...
            case "northwest":
                this.viewport.x = THREE.MathUtils.clamp(this.previousViewport.x + increment.x, 0, this.previousViewport.x + this.previousViewport.width - this.viewportWidthMin);
                this.viewport.height = THREE.MathUtils.clamp(this.previousViewport.height + increment.y, this.viewportHeightMin, this.window.innerHeight - this.viewport.y);
                this.snapViewport(frame);
                this.viewport.width = this.previousViewport.width - (this.viewport.x - this.previousViewport.x);
                break;
            case "west":
            ...
            case "southeast":
                this.viewport.y = THREE.MathUtils.clamp(this.previousViewport.y + increment.y, 0, this.previousViewport.y + this.previousViewport.height - this.viewportHeightMin);
                this.viewport.width = THREE.MathUtils.clamp(this.previousViewport.width + increment.x, this.viewportWidthMin, this.window.innerWidth - this.viewport.x);
                this.snapViewport(frame);
                this.viewport.height = this.previousViewport.height - (this.viewport.y - this.previousViewport.y);
                break;
            case "northeast":
    ...
    setViewport(viewport) { // Expressed in fraction of window width and window height
        ...
        this.viewport = this.percentageToPixels(this.currentViewport, this.window.innerWidth, this.window.innerHeight);
        this.viewportWidthMin = Math.round(this.viewportSizeMin * this.window.innerWidth);
        this.viewportHeightMin = Math.round(this.viewportSizeMin * this.window.innerHeight);
        if (this.view == "mini-map") {
